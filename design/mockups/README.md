...
## Letter-learning game concepts (September 24–25, 2026)

Early directions for a bilingual Hebrew + English letter game, generated with `scripts/gen-game-mockups.py` (`gemini-3-pro-image`). All four were rejected in favour of the interactive prototypes in `design/prototypes/`:

- `A_sorting-basket.jpg`, `B_listening-window.jpg`, `C_action-path.jpg`: Sorting Basket, Listening Window and Action Path.
- `D_all-three-comparison.jpg`: side-by-side comparison of the three.

The current direction is the Magic Brush Painter prototype (`design/prototypes/merged-letter-painter.html`): freehand letter tracing with flowing arrow guides and ordered stroke validation, where each traced letter reveals part of a watercolor storybook illustration (`design/prototypes/assets/storybook_apple.jpg`, generated by `scripts/gen-storybook-apple.py`).
