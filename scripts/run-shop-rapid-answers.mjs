...
try {
  await new Promise((resolve, reject) => {
    ...
    server.once('exit', (code, signal) => reject(new Error(`Rapid-answers test server exited: ${signal ?? code}`)));
    lines.on('line', line => {
      if (line === 'Local D1 browser acceptance: http://127.0.0.1:8788') resolve();
    });
  });
  execFileSync(process.execPath, ['scripts/test-shop-rapid-answers.mjs'], { cwd, stdio: 'inherit' });
} finally {
  lines.close();
