...
try {
  ...
  for(let stage=1;stage<=5;stage++) {
    ...
    if(activity==='memory-garden') {
      ...
      evaluate(`(async()=>{${setup}const {MAGIC_HOUSE_REQUESTS}=await import('/prototype/shared/magic-house-content.mjs');for(let i=0;i<55;i++){if(d.querySelector('#celebration').classList.contains('is-visible')){d.querySelector('#replay-button').click();return true;}const state=JSON.parse(window.bubblesSaveClient.getItem('house-round-forest-lotem-en-trail-house-1'));const request=MAGIC_HOUSE_REQUESTS.find(r=>r.id===state.requestIds[state.completedRequests]);if(request){for(const t of request.targets){const object=d.querySelector('.object-button[data-object-id="'+t.objectId+'"]');if(object&&!object.disabled){object.click();d.querySelector('[data-zone="'+t.zoneId+'"]').click();}}}await sleep(500);}throw Error('House did not complete');})()`);
    }
    browser('wait','--fn',`!document.querySelector('#activity-overlay').classList.contains('is-visible')`);
