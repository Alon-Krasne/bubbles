...
**Generated trail**:
The live learning route produced by `generateTrail()` from chapters, vocabulary categories, and game parameter recipes. It is deterministic and validated: six chapters × six stages = 36 stages whose Memory, Shop, Magic House and Reveal the Magic levels scale in difficulty. The four games appear nine times each in a varied sequence. Reveal the Magic earns normal route stars and uses letter guessing in the selected learning language. Video milestones stay after stages 5/10/15/20/25/30/35 and at the finale. The legacy hand-placed 15-stage route remains exported as `LEGACY_TRAIL_STAGES` for reference only.
