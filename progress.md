...
## September 13 Hebrew activities now work without reading

- User report: the Hebrew path could not be played by a child who speaks Hebrew but cannot read, and no Hebrew voice played. The previous language policy made Hebrew prompts and choices written-only (`prompt: written-hebrew`, `choices: written-hebrew`) and gated every speaker behind English.
- New policy: `LANGUAGE_POLICIES.hebrew` is `prompt: spoken-hebrew`, `choices: semantic-images`, `target: hebrew-without-answer-image`. Memory target cards auto-speak their Hebrew word on reveal and gained a replay button; the Shop speaks the composed Hebrew order (`אפשר בבקשה` + word + optional `בכמות` + number + `וגם`), shows pictured choices with Hebrew labels, and exposes the replay control; Magic House plays its Hebrew request recording and shows pictured drawer objects. Success audio uses the committed Hebrew `תודה`.
- Added 222 committed Hebrew TTS clips in `src/assets/audio/vocabulary/he` (214 words + 8 UI fragments) generated by `scripts/generate-hebrew-vocabulary-tts.mjs`; every clip is regenerated until `gemini-3.8-flash` confirms it pronounces the exact transcript. `generate-magic-house-tts.js` also emits Hebrew female request recordings so the spoken sentence matches the profile's grammatical gender.
...
- Review fix (v7.21.4): `holdMemoryBoard` was not cancelled, so reshuffling during the 650 ms hold celebrated the fresh, unfinished board. Added a `memoryRoundToken` that `startMemoryRound` and every exit path bump via `cancelMemoryCompletion`; the pending win chain calls `celebrateIfStillCurrent` and stands down when the token moved. Red receipt (guard disabled): `AssertionError ... {"celebrationVisible":true,"faceUp":0,"progress":"0 מתוך 4 זוגות"}`; green: `{"celebrationVisible":false,"faceUp":0,"progress":"0 מתוך 4 זוגות"}`.
- Shop coverage closed in `test:shop-rapid-answers`: dropped the seeded `route-*` so completion exercises the missing-route fallback (and now asserts the fresh route records `progress["32"]`), and added a phase that captures the spoken request and asserts the composed Hebrew sequence `can-i-have + item + and + item` (receipt: `can-i-have-D5Jc221m.mp3, water-O7HQv9-G.mp3, and-Dmm8Gt_K.mp3, teddy-bear-CPbTYntW.mp3`).
- Regression gates re-run: `npx tsc --noEmit`, `npm run build`, `test:fast-start`, `test:trail`, `test:memory-completion`, `test:memory-saves`, `test:memory-hebrew-board`, `test:shop-rapid-answers`, `test:shop-order-hint`, `test:store-audio`, `test:shop-art`.
